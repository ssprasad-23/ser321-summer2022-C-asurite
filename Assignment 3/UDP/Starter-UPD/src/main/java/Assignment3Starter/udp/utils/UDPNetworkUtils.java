...
// https://mkyong.com/java/java-convert-byte-to-int-and-vice-versa/
  ...
  public static void Send(DatagramSocket sock, InetAddress addr, int port, byte... bytes) throws IOException {
    ...
    while (offset < bytes.length) {
      ...
      byte[] totalBytes = UDPNetworkUtils.intToBytes(packetsTotal);
      byte[] currentBytes = UDPNetworkUtils.intToBytes(packetNum);
      byte[] lengthBytes = UDPNetworkUtils.intToBytes(length);
  ...
  static class Packet {
    ...
    public Packet(DatagramPacket packet) {
      ...
      Total = UDPNetworkUtils.bytesToInt(totalBytes);
      
      byte[] currentBytes = new byte[4];
      System.arraycopy(packet.getData(), 4, currentBytes, 0, 4);
      Current = UDPNetworkUtils.bytesToInt(currentBytes);
      
      byte[] lengthBytes = new byte[4];
      System.arraycopy(packet.getData(), 8, lengthBytes, 0, 4);
      Length = UDPNetworkUtils.bytesToInt(lengthBytes);
